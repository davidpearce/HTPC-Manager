...
class Services:
    ...
    def getiotop(self):
        ...
        output = temp2.replace("\n", "<br/>").replace("<br/></td>", "</td></tr><tr>").replace("</td><td>M/s", "M/s").replace("</td><td>K/s","K/s").replace("</td><td>B/s","B/s")_
            .replace("DISK</td><td>WRITE", "Disk Write").replace("DISK</td><td>READ", "Disk Read").replace("</td><td>%", "%").replace("Total</TD><TD>Disk", "Total Disk")
        return output